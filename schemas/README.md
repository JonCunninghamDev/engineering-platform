...
- `execution-envelope-v1.schema.json` defines bounded consumer execution authority, including independent worker and executor identities, an immutable base commit, branch route, scope, actions, checks, human gates, budgets, and an explicit prohibition on release writes.
- `worker-execution-result-v1.schema.json` defines provider-neutral execution evidence returned by a managed worker, including executor identity, immutable repository commits, changed paths, check exit/timing evidence, budget consumption, and blocked consequential actions.
